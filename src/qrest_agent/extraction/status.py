...
def _schema_messages(data: Any, name: str) -> list[str]:
    validator = Draft202012Validator(_asset_schema(name))
    return [
        f"{name}: {error.message}" for error in sorted(
...
def _requirement_issues(facts: list[dict]) -> list[dict]:
    ...
    if not isinstance(elevation, list) or not elevation:
        blocking(
            "building.elevations",
            f"Required for {EXPORT_REQUIREMENTS['building.elevations']}; "
            "elevation values are unavailable.",
        )
    ...
    if not isinstance(channels, list) or not channels:
        blocking(
            "monitoring.channels",
            "Channel definitions are required for strict qREST_DATA export.",
        )
    else:
        ...
        if incomplete:
            blocking(
                "monitoring.channels",
                f"{len(incomplete)} channel definitions are incomplete "
                f"(missing one of {', '.join(CHANNEL_REQUIRED_FIELDS)}): "
                + ", ".join(f"Channels[{i}]" for i in incomplete[:10]),
                issue_type="partial",
            )
        if isinstance(count, int) and not isinstance(count, bool) and count != len(channels):
            blocking(
                "monitoring.channels",
                f"Channel count fact is {count} but {len(channels)} definitions "
                "are recorded; do not drop either value.",
                issue_type="partial",
            )
    ...
    if not isinstance(start, str) or not start:
        blocking(
            "data.start_time",
            "Record start time is required by the strict qREST_DATA contract "
            "(DataInfo.StartTime).",
        )
    return issues
...
def evaluate_state(facts_data: dict, issues_data: dict) -> ReadinessResult:
    messages: list[str] = []
    facts: list[dict] = []
    issues: list[dict] = []
    ...
    if not messages:
        messages.extend(_schema_messages(facts_data, FACTS_SCHEMA_NAME))
        messages.extend(_schema_messages(issues_data, ISSUES_SCHEMA_NAME))
...
def render_status(result: ReadinessResult) -> str:
